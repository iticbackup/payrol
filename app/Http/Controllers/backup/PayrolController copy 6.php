...
class PayrolController extends Controller
{
    ...
    public function borongan_slip_gaji($kode_pengerjaan)
    {
        ...
        foreach ($query_weeklys as $key => $qw) {
            ...
            $row2_weekly = PengerjaanWeekly::select([
                        ...
                        ->first();
            // dd($row2_weekly);
            $row2_nama = BiodataKaryawan::where('nik',$row2_weekly->nik)->orderBy('nama','asc')->first();
            $row2_upah_lembur = [];
            $row2_total_upah_hasil_kerja = [];
            $row2_pengerjaans = Pengerjaan::where('operator_karyawan_id',$row2_weekly->operator_karyawan_id)
                                        ->where('kode_pengerjaan',$kode_pengerjaan)
                                        ->get();
            ...
            $pdf->Cell(35,5,'Nama','L',0,'L'); 
            $pdf->SetFont('Arial','B',7);
            $pdf->Cell(60,5,strtoupper($row2_nama->nama).' ('.$row2_nama->nik.')','R',0,'L');
            $pdf->ln(4);
    ...
    public function harian_slip_gaji($kode_pengerjaan)
    {
        ...
        foreach ($pengerjaan_harians as $key => $ph) {
            ...
            //                             ->orderBy('biodata_karyawan.nama','asc')
            //                             ->first();
            // echo json_encode($row2_weekly->nik);
            // $row2_nama = BiodataKaryawan::where('nik',$row2_weekly->nik)->orderBy('nama','asc')->first();
            // dd($ganjil_genap_second);
    ...
    public function supir_rit_slip_gaji($kode_pengerjaan)
    {
        ...
        $first_pengerjaan_supir_rit_id = PengerjaanRITWeekly::select([
                                                            ...
                                                            ->first();
        // dd($first_pengerjaan_supir_rit_id);
        // dd($pengerjaan_rit_weeklys);
        if ($first_pengerjaan_supir_rit_id->id %2 == 0) {
        ...
        $query_weeklys = PengerjaanRITWeekly::select([
                                                'pengerjaan_supir_rit_weekly.id as id',
        ...
        foreach ($query_weeklys as $key => $qw) {
            ...
            ### LINE 1 ###
            $pdf->SetFont('Arial','B',9);
            $pdf->Cell(95,5,'GAJI RIT-RITAN '.strtoupper($qw->nama),'LTR',0,'C'); 
            
            $pdf->Cell(2,5,'','LF',0,'L');
            
            $pdf->SetFont('Arial','B',9);
            $pdf->Cell(95,5,'GAJI RIT-RITAN '.strtoupper($row2_weekly->nama),'LTR',0,'C'); 
            $pdf->ln(4);
            ...
            $row1_pengerjaan_rit_dailys = PengerjaanRITHarian::where('karyawan_supir_rit_id',$qw->karyawan_supir_rit_id)
                                                            ->where('kode_pengerjaan',$kode_pengerjaan)
                                                            ->get();
            $row1_upah_dasar = array();
            $row2_pengerjaan_rit_dailys = PengerjaanRITHarian::where('karyawan_supir_rit_id',$row2_weekly->karyawan_supir_rit_id)
                                                                ->where('kode_pengerjaan',$kode_pengerjaan)
                                                                ->get();
            // dd($row2_pengerjaan_rit_dailys);
            $row2_upah_dasar = array();
            foreach ($row1_pengerjaan_rit_dailys as $row1_pengerjaan_rit_daily) {
                if (empty($row1_pengerjaan_rit_daily->hasil_kerja_1)) {
                    // $jenis_umk = 0;
                    $row1_hasil_kerja_1 = 0;
                    $row1_hasil_umk_rit = 0;
                    $row1_tarif_umk = 0;
                    $row1_dpb = 0;
                    $row1_jenis_umk = '-';
                }else{
                    $row1_explode_hasil_kerja_1 = explode("|",$row1_pengerjaan_rit_daily->hasil_kerja_1);
                    $row1_umk_rit = RitUMK::where('id',$row1_explode_hasil_kerja_1[0])->first();
                    // dd($row1_umk_rit);
                    if (empty($row1_umk_rit)) {
                        $row1_hasil_kerja_1 = 0;
                        $row1_hasil_umk_rit = 0;
                        $row1_tarif_umk = 0;
                        $row1_dpb = 0;
                        ...
                        $row1_hasil_kerja_1 = $row1_umk_rit->tarif;
                        $row1_hasil_umk_rit = $row1_umk_rit->kategori_upah;
                        $row1_tarif_umk = $row1_umk_rit->tarif;
                        $row1_dpb = $row1_pengerjaan_rit_daily->dpb/7*$row1_pengerjaan_rit_daily->upah_dasar;
                        if (empty($row1_umk_rit->rit_tujuan)) {
                            $row1_jenis_umk = '-';
                        }else{
                            $row1_jenis_umk = $row1_umk_rit->rit_tujuan->tujuan.' - '.$row1_umk_rit->rit_kendaraan->jenis_kendaraan;
                        }
                    }
                    $row1_total_upah_dasar = $row1_tarif_umk+$row1_dpb;
                    array_push($row1_upah_dasar,$row1_total_upah_dasar);
                }

                if (empty($row2_pengerjaan_rit_daily->hasil_kerja_1)) {
                    $row2_hasil_kerja_1 = 0;
                    $row2_hasil_umk_rit = 0;
                    $row2_tarif_umk = 0;
                    $row2_dpb = 0;
                    $row2_jenis_umk = '-';
                }else{
                    $row2_explode_hasil_kerja_1 = explode("|",$row2_pengerjaan_rit_daily->hasil_kerja_1);
                    $row2_umk_rit = RitUMK::where('id',$row2_explode_hasil_kerja_1[0])->first();
                    if (empty($row2_umk_rit)) {
                        $row2_hasil_kerja_1 = 0;
                        $row2_hasil_umk_rit = 0;
                        $row2_tarif_umk = 0;
                        $row2_dpb = 0;
                        ...
                        $row2_hasil_kerja_1 = $row2_umk_rit->tarif;
                        $row2_hasil_umk_rit = $row2_umk_rit->kategori_upah;
                        $row2_tarif_umk = $row2_umk_rit->tarif;
                        $row2_dpb = $row2_pengerjaan_rit_daily->dpb/7*$row2_pengerjaan_rit_daily->upah_dasar;
                        if (empty($row2_umk_rit->rit_tujuan)) {
                            $row2_jenis_umk = '-';
                        }else{
                            $row2_jenis_umk = $row2_umk_rit->rit_tujuan->tujuan.' - '.$row2_umk_rit->rit_kendaraan->jenis_kendaraan;
                        }
                    }
                    $row2_total_upah_dasar = $row2_tarif_umk+$row2_dpb;
                    array_push($row2_upah_dasar,$row2_total_upah_dasar);
                }

                ### LINE 4 ###
                $pdf->SetFont('Arial','',9);
                $pdf->Cell(20,5,Carbon::parse($row1_pengerjaan_rit_daily->tanggal_pengerjaan)->isoFormat('D MMMM'),'L',0,'C'); 
                $pdf->Cell(53,5,'DPB','LR',0,'L');
                $pdf->Cell(22,5,number_format($row1_dpb,0,',','.'),'R',0,'R');
                
                // $pdf->Cell(2,5,Carbon::parse($row2_pengerjaan_rit_dailys->tanggal_pengerjaan)->isoFormat('D MMMM'),'LF',0,'L');
                $pdf->Cell(2,5,'','LF',0,'L');
                // dd($row2_pengerjaan_rit_dailys);
                $pdf->SetFont('Arial','',9);
                $pdf->Cell(20,5,'','L',0,'C'); 
                $pdf->Cell(53,5,'DPB','LR',0,'L');
                $pdf->Cell(22,5,number_format(0,0,',','.'),'R',0,'R');
                $pdf->ln(4);

                ### LINE 5 ###
                $pdf->SetFont('Arial','',7);
                $pdf->Cell(20,5,'','BLR',0,'C'); 
                $pdf->Cell(53,5,$row1_jenis_umk,'BLR',0,'L');
                $pdf->SetFont('Arial','',9);
                $pdf->Cell(22,5,number_format($row1_hasil_kerja_1,0,',','.'),'BLR',0,'R');
                
                $pdf->Cell(2,5,'','LF',0,'L');
                
                $pdf->SetFont('Arial','',7);
                $pdf->Cell(20,5,'','BLR',0,'C'); 
                $pdf->Cell(53,5,'','BLR',0,'L');
                $pdf->SetFont('Arial','',9);
                $pdf->Cell(22,5,number_format(0,0,',','.'),'BLR',0,'R');
                $pdf->ln(5);
            }
            ...
            //     $row1_hasil_lembur = 0;
            // }else{
            //     $row1_explode_lembur = explode("|",$qw->lembur);
