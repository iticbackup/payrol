...
class PayrolController extends Controller
{
    ...
    public function harian_slip_gaji($kode_pengerjaan)
    {
        ...
        $first_pengerjaan_harian_id = PengerjaanHarian::select([
                                            ...
                                            ->orderBy('biodata_karyawan.nama','asc')
                                            ->first();
        // dd($first_pengerjaan_harian_id);
        if ($first_pengerjaan_harian_id->id %2 == 0) {
            $ganjil_genap = 'MOD(pengerjaan_harian.id, 2) = 0';
        }else{
            $ganjil_genap = 'MOD(pengerjaan_harian.id, 2) <> 0';
        }

        // $query_weeklys = PengerjaanHarian::select([
        //                                         'pengerjaan_harian.id as id',
        //                                         'pengerjaan_harian.operator_harian_karyawan_id as operator_harian_karyawan_id',
        //                                         'operator_harian_karyawan.nik as nik',
        //                                         'biodata_karyawan.nama as nama',
        ...
        //                                     ->orderBy('biodata_karyawan.nama','asc')
        //                                     ->get();
        $query_weeklys = DB::table('pengerjaan_harian')
                            ->select([
                                'pengerjaan_harian.id as id',
                                'pengerjaan_harian.operator_harian_karyawan_id as operator_harian_karyawan_id',
                                'operator_harian_karyawan.nik as nik',
        ...
        foreach ($query_weeklys as $qw) {
            ...
            if (empty($qw->lembur)) {
                ...
                $row1_exlode_lembur = explode("|",$qw->lembur);
            ...
            $row1_total_gaji_diterima = ($qw->upah_dasar_weekly+$row1_hasil_lembur+$row1_tunjangan_kehadiran+$row1_tunjangan_kerja+
                                    $row1_plus_1+$row1_plus_2+$row1_plus_3+$qw->uang_makan)-
                                    ($row1_jht+$row1_bpjs_kesehatan+$row1_minus_1+$row1_minus_2);
            ###############endrow 1
            ...
            // $row2_weekly = DB::table('pengerjaan_harian')
            //                     ->leftJoin('operator_harian_karyawan','operator_harian_karyawan.id','=','pengerjaan_harian.operator_harian_karyawan_id')
            //                     ->where('pengerjaan_harian.kode_pengerjaan',$kode_pengerjaan)
            //                     ->first();
            // dd($row2_weekly);
            // dd($decode_row2_weekly);
            // $row2_nama = BiodataKaryawan::where('nik',$row2_weekly->nik)->orderBy('nama','asc')->first();
            // dd($row2_weekly);
            ...
            $pdf->Cell(35,5,'Nama','L',0,'L'); 
            $pdf->SetFont('Arial','B',7);
            $pdf->Cell(60,5,strtoupper('').' ('.$row2_weekly->id.')','R',0,'L');
            $pdf->ln(4);
        }
