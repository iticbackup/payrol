...
class CreatePengerjaanSupirRitTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('pengerjaan_supir_rit', function (Blueprint $table) {
            // $table->id();
            $table->integer('id')->primary();
            $table->string('kode_pengerjaan');
            $table->string('kode_payrol');
            $table->integer('karyawan_supir_rit_id')->unsigned();
            $table->date('tanggal_pengerjaan');
            $table->string('hasil_kerja_1')->nullable();
            $table->string('hasil_kerja_2')->nullable();
            $table->string('dpb')->nullable();
            $table->string('upah_dasar')->nullable();
            $table->timestamps();
            $table->softDeletes();
        });
        Schema::create('pengerjaan_supir_rit_weekly', function (Blueprint $table) {
            // $table->id();
            $table->integer('id')->primary();
            $table->string('kode_pengerjaan');
            $table->string('kode_payrol');
    ...
    {
        Schema::dropIfExists('pengerjaan_supir_rit');
        Schema::dropIfExists('pengerjaan_supir_rit_weekly');
    }
}
