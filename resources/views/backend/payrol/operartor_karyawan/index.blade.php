...
@section('content')
    ...
    @include('backend.payrol.operartor_karyawan.modalBuat')
    @include('backend.payrol.operartor_karyawan.modalEdit')
    ...
    <script>
        ...
        var table = $('#datatables').DataTable({
            ...
            columns: [
                ...
                {
                    data: 'jenis_operator_id',
                    name: 'jenis_operator_id'
                },
                {
        ...
        function edit(id) {
            var url = '{{ route('operator_karyawan.detail', ':id') }}';
            ...
            $.ajax({
                ...
                success: (result) => {
                    ...
                    $('.edit_jenis_operator_id').val(result.data.jenis_operator_id);
                    $('.edit_jenis_operator_detail_id').val(result.data.jenis_operator_detail_id);
                    $('.edit_jenis_operator_detail_pekerjaan_id').val(result.data.jenis_operator_detail_pekerjaan_id);
                    $('#edit_jht').val(result.data.jht);
                    $('#edit_bpjs').val(result.data.bpjs);
                    $('#edit_training').val(result.data.training);
        ...
        $('.buat_nik_karyawan').on('change', function() {
            axios.post('{{ route('operator_karyawan.select_biodata_karyawan') }}', {
                ...
                .then(function(response) {
                    ...
                    // $('#buat_nama_karyawan').val(response.data.nama);
        ...
        $('.posisi_pengerjaan').on('click', function() {
            axios.post('{{ route('operator_karyawan.select_jenis_operator_detail_pekerjaan') }}', {
                    id: $(this).val()
                })
                .then(function(response) {
                    $('.posisi_pekerjaan').empty();

                    $.each(response.data, function(id, jenis_posisi_pekerjaan) {
                        // alert(nama);
                        $('.posisi_pekerjaan').append(new Option(jenis_posisi_pekerjaan, id));
                    })
                });
        });

        // $('.edit_buat_nik_karyawan').on('change', function() {
        //     axios.post('{{ route('operator_karyawan.select_biodata_karyawan') }}', {
        //             nik: $(this).val()
        //         })
        //         .then(function(response) {
        //             $('.edit_buat_nama_karyawan').val(response.data.nama);
        //             // $('#posisi_pengerjaan').empty();
        ...
        // $('.edit_posisi').on('change', function() {
        //     axios.post('{{ route('operator_karyawan.select_jenis_operator_detail') }}', {
        //             id: $(this).val()
        //         })
        //         .then(function(response) {
        //             $('.edit_posisi_pengerjaan').empty();

        //             $.each(response.data, function(id, jenis_posisi) {
        //                 // alert(nama);
        //                 $('.edit_posisi_pengerjaan').append(new Option(jenis_posisi, id));
        //             })
        //         });
        // });

        // $('.edit_posisi_pengerjaan').on('change', function() {
        //     axios.post('{{ route('operator_karyawan.select_jenis_operator_detail_pekerjaan') }}', {
        //             id: $(this).val()
        //         })
        //         .then(function(response) {
        //             $('.edit_posisi_pekerjaan').empty();

        //             $.each(response.data, function(id, jenis_posisi_pekerjaan) {
        //                 // alert(nama);
        //                 $('.edit_posisi_pekerjaan').append(new Option(jenis_posisi_pekerjaan, id));
        //             })
        //         });
        // });
        ...
        $('#form-simpan').submit(function(e) {
            ...
            $.ajax({
                type: 'POST',
                url: "{{ route('operator_karyawan.simpan') }}",
                data: formData,
                contentType: false,
                processData: false,
                success: (result) => {
        ...
        $('#form-update').submit(function(e) {
            ...
            $.ajax({
                type: 'POST',
                url: "{{ route('operator_karyawan.update') }}",
                data: formData,
                contentType: false,
                processData: false,
                success: (result) => {
