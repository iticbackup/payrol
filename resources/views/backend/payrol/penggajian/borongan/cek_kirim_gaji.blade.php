...
@section('title')
    Cek Kirim Gaji Payrol Borongan
@endsection
                        ...
                        <tbody>
                            @foreach ($check_kirim_gajis as $key => $check_kirim_gaji)
                                <tr>
                                    ...
                                    <td>{{ $check_kirim_gaji->karyawan_operator->jenis_operator->jenis_operator.' - '.$check_kirim_gaji->karyawan_operator->jenis_operator_detail_pengerjaan->jenis_posisi_pekerjaan }}</td>
                                    ...
                                    <td>
                                        <button type="button" class="btn btn-primary" onclick="resend_mail(`{{ $new_data_pengerjaan->kode_pengerjaan }}`,{{ $check_kirim_gaji->id }})">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 512 512">
...
<script src="{{ URL::asset('public/assets/plugins/datatables/jquery.dataTables.min.js') }}"></script>
    ...
    <script>
        ...
        function resend_mail(kode_pengerjaan,id)
        {
            $.ajax({
                type: 'GET',
                url: "{{ url('payrol/borongan/') }}"+'/'+kode_pengerjaan+'/detail_kirim_email/'+id+'/kirim_ulang',
                contentType: false,
                processData: false,
                beforeSend: () => {
                    let timerInterval;
