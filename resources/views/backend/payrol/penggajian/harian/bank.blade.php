...
<body>
    ...
    <div>
        <b>Rekap Gaji Karyawan Harian PT Indonesian Tobacco Tbk.</b> <br>
        <b>Tanggal :
    ...
    <table>
        ...
        @foreach ($pengerjaan_harians as $key=> $pengerjaan_harian)
            @php
                array_push($total_nominal, $pengerjaan_harian->upah_dasar_weekly);
            @endphp
            <tr>
                <td style="text-align: center">{{ $key+1 }}</td>
                <td style="text-align: center">{{ $pengerjaan_harian->nik }}</td>
                <td>{{ $pengerjaan_harian->nama }}</td>
                <td style="text-align: center">{{ $pengerjaan_harian->rekening }}</td>
                <td style="text-align: right">{{ number_format($pengerjaan_harian->upah_dasar_weekly,0,',','.') }}</td>
